    ...
    public class GetInquiryByAddressController : ControllerBase
    {
        ...
        public ActionResult Get(string address, bool mainnet = true, bool verbose = false)
        {
            ...
            if (Regex.IsMatch(address, pattern))
            {
                ...
                if (mainnet)
                {
                    arguments = "--versionbyte " + wrapper.ProdVersionByte + " --getinquirybyaddress --password " + wrapper.ProdRPCPassword + " --url " + wrapper.ProdRPCURL + " --username " + wrapper.ProdRPCUser + " --address " + address;
                    if (verbose) { arguments = arguments + " --verbose"; }
                    result = wrapper.RunCommand(wrapper.ProdCLIPath, arguments);
                }
                else { arguments = "--versionbyte " + wrapper.TestVersionByte + " --getinquirybyaddress --password " + wrapper.TestRPCPassword + " --url " + wrapper.TestRPCURL + " --username " + wrapper.TestRPCUser + " --address " + address;
                    if (verbose) { arguments = arguments + " --verbose"; }
