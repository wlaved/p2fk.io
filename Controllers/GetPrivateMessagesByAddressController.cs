    ...
    public class GetPublicMessagesByAddressController : ControllerBase
    {
        ...
        public ActionResult Get(string address, int skip=0, int qty = 10, bool mainnet = true)
        {
            ...
            if (Regex.IsMatch(address, pattern))
            {
                ...
                if (mainnet)
                {
                    arguments = "--versionbyte " + wrapper.ProdVersionByte + " --getpublicmessagesbyaddress --password " + wrapper.ProdRPCPassword + " --url " + wrapper.ProdRPCURL + " --username " + wrapper.ProdRPCUser + " --skip " + skip + " --qty " + qty + " --address " + address;
                    result = wrapper.RunCommand(wrapper.ProdCLIPath, arguments);
                }
                else { arguments = "--versionbyte " + wrapper.TestVersionByte + " --getpublicmessagesbyaddress --password " + wrapper.TestRPCPassword + " --url " + wrapper.TestRPCURL + " --username " + wrapper.TestRPCUser + " --skip " + skip + " --qty " + qty + " --address " + address;
